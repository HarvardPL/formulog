...
public final class Unification {
	...
	public static boolean canBindVars(ComplexLiteral atom, Set<Var> boundVars, Map<Var, Integer> varCounts)
	...
	private static boolean handleNormalAtom(UserPredicate atom, Set<Var> boundVars, Map<Var, Integer> varCounts) {
		if (atom.isNegated()) {
			// Allow anonymous variables in negated atoms.
			Set<Var> vars = atom.varSet().stream().filter(v -> !Integer.valueOf(1).equals(varCounts.get(v)))
	...
	// public static boolean unify(Atom a1, Atom a2, Substitution s) throws
	// EvaluationException {
	...
	// }
	//
	// public static void unsafeUnifyWithFact(NormalAtom atom, NormalAtom fact,
	// Substitution s) {
	...
	private static boolean unify2(Term t1, Term t2, Substitution s) throws EvaluationException {
		t1 = t1.applySubstitution(s);
		t2 = t2.applySubstitution(s);
	 ...
	 * This assumes that the two terms are in a restricted form...
	 */
	private static boolean unify2Iterative(Term t1, Term t2, Substitution s) throws EvaluationException {
		Deque<Term> ts1 = new ArrayDeque<>();
		Deque<Term> ts2 = new ArrayDeque<>();
		ts1.add(t1);
		ts2.add(t2);
		while (!ts1.isEmpty()) {
	 ...
	 */
	// @SuppressWarnings("unused")
	// private static boolean unify1(Term t1, Term t2, Substitution s) throws
	// EvaluationException {
	...
	// private final Graph<Node, DefaultEdge> ugraph = new
	// DefaultDirectedGraph<>(DefaultEdge.class);
	// // Invariant: a term is in dgraph iff it is not ground.
	// private final Graph<Node, DefaultEdge> dgraph = new
	...
	// // ugraph might not contain vertex if it is coalesced in a previous
	// iteration.
	...
	// }
	//
	// private boolean unifyGroundTermWithOther(Term grndT, Term other, Set<Node>
	// frontier) {
	...
	// return dgraph.outgoingEdgesOf(src).stream().map(e ->
	// dgraph.getEdgeTarget(e)).collect(Collectors.toSet());
	// }
	//
