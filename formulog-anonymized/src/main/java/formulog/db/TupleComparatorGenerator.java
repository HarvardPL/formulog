...
import formulog.util.IntArrayWrapper;
...
public class TupleComparatorGenerator extends ClassLoader {
	...
	private static final Type termType = new ObjectType("formulog.ast.Term");
	...
	public Comparator<Term[]> generate1(int[] accessPat) throws InstantiationException, IllegalAccessException {
		String className = "formulog.db.CustomComparator" + cnt.getAndIncrement();
	...
	private InstructionList genLoad(InstructionFactory f, int argn, int idx) {
		...
		il.append(f.createInvoke("formulog.ast.Term", "getId", Type.INT, new Type[] {}, Const.INVOKEINTERFACE));
